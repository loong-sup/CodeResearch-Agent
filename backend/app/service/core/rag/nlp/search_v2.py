...
class Dealer:
    ...
    def search(self, req, idx_names: str | list[str],
        ...
        src = req.get("fields",
                      ["docnm_kwd", "content_ltks", "kb_id", "repository_id", "user_id", "img_id", "title_tks", "title_sm_tks",
                       "file_path_kwd", "file_path_tks", "symbol_kwd", "symbol_tks", "chunk_kind_kwd",
                       "start_line_int", "end_line_int", "language_kwd", "important_kwd", "position_int",
    ...
    def rerank(self, sres, query, tkweight=0.3,
        ...
        for i in sres.ids:
            ...
            file_path_tks = [t for t in sres.field[i].get("file_path_tks", "").split() if t]
            symbol_tks = [t for t in sres.field[i].get("symbol_tks", "").split() if t]
            question_tks = [t for t in sres.field[i].get("question_tks", "").split() if t]
            important_kwd = sres.field[i].get("important_kwd", [])
            tks = content_ltks + title_tks * 2 + file_path_tks * 4 + symbol_tks * 6 + important_kwd * 5 + question_tks * 6
            ins_tw.append(tks)
    ...
    def rerank_by_model(self, rerank_mdl, sres, query, tkweight=0.3,
        ...
        for i in sres.ids:
            ...
            symbol_tks = [t for t in sres.field[i].get("symbol_tks", "").split() if t]
            important_kwd = sres.field[i].get("important_kwd", [])
            tks = content_ltks + title_tks + file_path_tks * 2 + symbol_tks * 3 + important_kwd
            ins_tw.append(tks)
    ...
    def retrieval(self, question, embd_mdl, tenant_ids, kb_ids, page, page_size, similarity_threshold=0.2,
        ...
        for i in idx:
            ...
            d = {
                ...
                "docnm_kwd": dnm,
                "kb_id": chunk["kb_id"],
                "file_path_kwd": chunk.get("file_path_kwd", dnm),
                "symbol_kwd": chunk.get("symbol_kwd", ""),
                "chunk_kind_kwd": chunk.get("chunk_kind_kwd", ""),
                "language_kwd": chunk.get("language_kwd", ""),
                "start_line_int": chunk.get("start_line_int", 1),
                "end_line_int": chunk.get("end_line_int", 1),
